...
2. **До распаковки** щёлкни по архиву правой кнопкой → «Свойства», внизу отметь «Разблокировать» и нажми «ОК».
   Так Windows не будет ругаться на игру при каждом запуске.
3. Распакуй архив в любую папку и запусти `Stazher.exe`.
4. Если всё же появится «Система Windows защитила ваш компьютер», нажми «Подробнее» → «Выполнить в любом случае».
   Предупреждение появляется потому, что игра пока не подписана сертификатом.
